...
const Comment: FunctionComponent<CommentProps> = ({ id, media_type }) => {
  ...
  useEffect(() => {
    ...
    getDocs(collection(db, `${media_type}-${id as number}`))
  }, [media_type, id]);
    ...
    <div className="mb-16">
      <div className="flex items-center justify-between mb-6">
        <div className="relative w-[140px]">
          <p className="md:text-2xl text-xl text-white font-medium">Коментарі</p>
      ...
      {commentData && commentData.size > commentLimit && (
        ...
        >
          Завантажити більше коментарів ({commentLimit}/{commentData.size})
        </button>
      )}
    </div>
  );
