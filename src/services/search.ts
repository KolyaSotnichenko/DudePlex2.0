...
export const getRecommendGenres2 =
  async (): Promise<getRecommendGenres2Type> => {
    const movieGenres = (await axios.get("/genre/movie/list", {params: {language: "uk-UA"}})).data.genres;
    const tvGenres = (await axios.get("/genre/tv/list", {params: {language: "uk-UA"}})).data.genres;
